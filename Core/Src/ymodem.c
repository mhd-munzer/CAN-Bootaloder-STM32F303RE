...
static HAL_StatusTypeDef CAN_Receive_data(uint8_t * data, uint32_t length, uint32_t Timeout);
static HAL_StatusTypeDef CAN_Send_data(uint8_t * data, uint32_t length);
...
static HAL_StatusTypeDef CAN_Receive_data(uint8_t * data, uint32_t length, uint32_t Timeout){
	...
	memset(Rxbuffer, 0, 8);
	if (length <= 8){
	  pRxHeader.DLC = length;
	  checking1:
	  level = HAL_CAN_GetRxFifoFillLevel(&hcan, CAN_RX_FIFO0);
	  if (level > 0){
		  status = HAL_CAN_GetRxMessage(&hcan, CAN_RX_FIFO0, &pRxHeader, &Rxbuffer[0]);
		  memcpy(data, Rxbuffer, 8 * sizeof(uint8_t));
		  return status;
	  }
	  else {
	  HAL_Delay(10);
      ...
      while (data_received < length){
    	  ...
    	  pRxHeader.DLC = data_size;
    	  checking:
    	  level = HAL_CAN_GetRxFifoFillLevel(&hcan, CAN_RX_FIFO0);
    	  if (level > 0){
    		  status = HAL_CAN_GetRxMessage(&hcan, CAN_RX_FIFO0, &pRxHeader, &Rxbuffer[0]);
		  if ( status == HAL_OK){
			  memcpy(data + data_received, &Rxbuffer[0], data_size * sizeof(uint8_t));
...
static HAL_StatusTypeDef CAN_Send_data(uint8_t * data, uint32_t length){
	...
	uint8_t data_size = 0;
	if (length <= 8){
	  pTxHeader.DLC = length;
	  HAL_CAN_AddTxMessage(&hcan, &pTxHeader, data, &TxMailBox);
	  HAL_Delay(150);
	}
	else{
		while (data_sent < length){
		  ...
		  pTxHeader.DLC = data_size;
		  memcpy(Txbuffer, data + data_sent, data_size);
		  //uint32_t lev = HAL_CAN_GetTxMailboxesFreeLevel(&hcan);
		  //uint32_t y = HAL_CAN_IsTxMessagePending(&hcan, TxMailBox);
	    if (HAL_CAN_AddTxMessage(&hcan, &pTxHeader, Txbuffer, &TxMailBox) == HAL_OK){
	    	data_left -= data_size;
	    	data_sent += data_size;
	    	HAL_Delay(50);
	}
...
static HAL_StatusTypeDef ReceivePacket(uint8_t *p_data, uint32_t *p_length, uint32_t timeout)
{
  ...
  //uint8_t char1;
  uint32_t Timeout = timeout/7;
  //uint32_t level = 0;
  uint8_t received_pckt[8];
  *p_length = 0;
  //status = HAL_UART_Receive(&UartHandle, &char1, 1, timeout);
  status = CAN_Receive_data(received_pckt, 1, Timeout);
  if (status == HAL_OK)
  {
    switch (received_pckt[0])
    {
      case SOH:
      ...
      case CA:
        //if ((HAL_UART_Receive(&UartHandle, &char1, 1, timeout) == HAL_OK) && (char1 == CA))
    	  Timeout = timeout / 7;
    	  status = CAN_Receive_data(received_pckt, 1, Timeout);
    	  if((status== HAL_OK) && (received_pckt[0] == CA))
        {
          packet_size = 2;
        }
        else
    ...
    //*p_data = received_pckt[0];
    *p_data = 0;
    memcpy(p_data + 1, received_pckt, 8);
    if (packet_size >= PACKET_SIZE )
    {
      //status = HAL_UART_Receive(&UartHandle, &p_data[PACKET_NUMBER_INDEX], packet_size + PACKET_OVERHEAD_SIZE, timeout);
    	Timeout = timeout / 7;
    	status = CAN_Receive_data(&p_data[8], packet_size + PACKET_OVERHEAD_SIZE, Timeout );
                ...
                if (packets_received == 0)
                {
                  ...
                  if (aPacketData[PACKET_DATA_INDEX] != 0)
                  {
                    ...
                    if (*p_size > (USER_FLASH_SIZE + 1))
                    {
                      ...
                      //HAL_UART_Transmit(&UartHandle, &tmp, 1, NAK_TIMEOUT);
                      pTxHeader.DLC = 1 ;
                      HAL_CAN_AddTxMessage(&hcan, &pTxHeader, &tmp, &TxMailBox);
                      HAL_CAN_AddTxMessage(&hcan, &pTxHeader, &tmp, &TxMailBox);
                      result = COM_LIMIT;
                    }
...
COM_StatusTypeDef Ymodem_Transmit (uint8_t *p_buf, const uint8_t *p_file_name, uint32_t file_size)
{
  ...
  while (( !ack_recpt ) && ( result == COM_OK ))
  {
    ...
    //HAL_UART_Transmit(&UartHandle, &aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE, NAK_TIMEOUT);
	  ...
	  status = CAN_Send_data(&aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE);
	  //if (status != HAL_OK)
    ...
    Txbuffer[0]= temp_crc >> 8;
    Txbuffer[1] = temp_crc & 0xFF;
    //Serial_PutByte(temp_crc >> 8);
    //Serial_PutByte(temp_crc & 0xFF);
    CAN_Send_data(Txbuffer, 2);
#else /* CRC16_F */   
    ...
    recieve_ACK:
    Timeout = 10000;
    status = CAN_Receive_data(&a_rx_ctrl[0], 1, Timeout );
    if( status == HAL_OK)
    //if (HAL_UART_Receive(&UartHandle, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK)
    {
      ...
      else if (a_rx_ctrl[0] == CA)
      {
        ...
        //if ((HAL_UART_Receive(&UartHandle, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == CA))
    	  status = CAN_Receive_data(&a_rx_ctrl[0], 1, Timeout);
		  if((status == HAL_OK) && (a_rx_ctrl[0] == CA))
        {
          HAL_Delay( 2 );
  ...
  while ((size) && (result == COM_OK ))
  {
    ...
    while (( !ack_recpt ) && ( result == COM_OK ))
    {
      ...
      status = CAN_Send_data(&aPacketData[PACKET_START_INDEX], pkt_size + PACKET_HEADER_SIZE);
      /* Send CRC or Check Sum based on CRC16_F */
#ifdef CRC16_F    
      ...
      if((CAN_Receive_data(&a_rx_ctrl[0], 1, Timeout ) == HAL_OK) && (a_rx_ctrl[0] == ACK))
  ...
  while (( !ack_recpt ) && ( result == COM_OK ))
  {
	...
	if(CAN_Receive_data(&a_rx_ctrl[0], 1, Timeout ) == HAL_OK)
    //if (HAL_UART_Receive(&UartHandle, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK)
    {
      if (a_rx_ctrl[0] == ACK)
      {
      ...
      else if (a_rx_ctrl[0] == CA)
      {
        ...
        //if ((HAL_UART_Receive(&UartHandle, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == CA))
	      if((CAN_Receive_data(&a_rx_ctrl[0], 1, Timeout ) == HAL_OK) && (a_rx_ctrl[0] == CA))
        {
          HAL_Delay( 2 );
          //__HAL_UART_FLUSH_DRREGISTER(&UartHandle);
          result = COM_ABORT;
  ...
  if ( result == COM_OK )
  {
    ...
    //HAL_UART_Transmit(&UartHandle, &aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE, NAK_TIMEOUT);
     ...
     CAN_Send_data(&aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE);
    /* Send CRC or Check Sum based on CRC16_F */
#ifdef CRC16_F
    temp_crc = HAL_CRC_Calculate(&CrcHandle, (uint32_t*)&aPacketData[PACKET_DATA_INDEX], PACKET_SIZE);
    //Serial_PutByte(temp_crc >> 8);
